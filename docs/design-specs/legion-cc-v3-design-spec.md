...
<div class="data-packet" data-type="intel">
  ...
  <div class="packet-content">Target Avatar: High-stress tech workers...</div>
</div>
```
...
export class StrategyCore {
  ...
  update(deltaTime) {
    ...
    if (this.state === "analyzing") {
      ...
      this.material.emissiveIntensity =
        0.4 + Math.sin(Date.now() * 0.005) * 0.2;
    }
  ...
  smoothReturnToIdle() {
    ...
    this.mesh.scale.x = THREE.MathUtils.lerp(
      this.mesh.scale.x,
      targetScale,
      0.1,
    );
    ...
    this.mesh.scale.z = THREE.MathUtils.lerp(
      this.mesh.scale.z,
      targetScale,
      0.1,
    );
